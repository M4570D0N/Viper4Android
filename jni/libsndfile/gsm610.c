...
static sf_count_t gsm610_write_s(SF_PRIVATE *psf, const short *ptr,
                                 sf_count_t len);
static sf_count_t gsm610_write_i(SF_PRIVATE *psf, const int *ptr,
                                 sf_count_t len);
static sf_count_t gsm610_write_f(SF_PRIVATE *psf, const float *ptr,
                                 sf_count_t len);
static sf_count_t gsm610_write_d(SF_PRIVATE *psf, const double *ptr,
                                 sf_count_t len);

static int gsm610_read_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
                             short *ptr, int len);
static int gsm610_write_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
...
int gsm610_init(SF_PRIVATE *psf) {
    ...
    if (psf->file.mode == SFM_READ) {
        ...
        else if (psf->datalength % pgsm610->blocksize
                        == 1&& pgsm610->blocksize == GSM610_BLOCKSIZE) {
            /*
...
static int gsm610_wav_decode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610) {
    ...
    if ((k = psf_fread(pgsm610->block, 1, WAV_W64_GSM610_BLOCKSIZE, psf))
    ...
    if (gsm_decode(pgsm610->gsm_data,
                   pgsm610->block + (WAV_W64_GSM610_BLOCKSIZE + 1) / 2,
                   pgsm610->samples + WAV_W64_GSM610_SAMPLES / 2) < 0) {
...
static int gsm610_decode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610) {
    ...
    if ((k = psf_fread(pgsm610->block, 1, GSM610_BLOCKSIZE, psf))
            != GSM610_BLOCKSIZE)
        psf_log_printf(psf, "*** Warning : short read (%d != %d).\n", k,
                       GSM610_BLOCKSIZE);
...
static int gsm610_read_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
    ...
    while (indx < len) {
        if (pgsm610->blockcount >= pgsm610->blocks
                && pgsm610->samplecount >= pgsm610->samplesperblock) {
        ...
        memcpy(&(ptr[indx]), &(pgsm610->samples[pgsm610->samplecount]),
               count * sizeof(short));
        indx += count;
...
static sf_count_t gsm610_seek(SF_PRIVATE *psf, int UNUSED (mode),
                              sf_count_t offset) {
    GSM610_PRIVATE *pgsm610;
    ...
    if (psf->file.mode == SFM_READ) {
        if (psf->read_current != newblock * pgsm610->samplesperblock + newsample) {
            psf_fseek(psf, psf->dataoffset + newblock * pgsm610->samplesperblock,
                      SEEK_SET);
...
static int gsm610_encode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610) {
    ...
    if ((k = psf_fwrite(pgsm610->block, 1, GSM610_BLOCKSIZE, psf))
                != GSM610_BLOCKSIZE)
...
static int gsm610_wav_encode_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610) {
    ...
    /* Encode the samples. */
    gsm_encode(pgsm610->gsm_data, pgsm610->samples, pgsm610->block);
    gsm_encode(pgsm610->gsm_data, pgsm610->samples + WAV_W64_GSM610_SAMPLES / 2,
               pgsm610->block + WAV_W64_GSM610_BLOCKSIZE / 2);

    /* Write the block to disk. */
    if ((k = psf_fwrite(pgsm610->block, 1, WAV_W64_GSM610_BLOCKSIZE, psf))
    != WAV_W64_GSM610_BLOCKSIZE)
    psf_log_printf(psf, "*** Warning : short write (%d != %d).\n", k,
                   WAV_W64_GSM610_BLOCKSIZE);
...
static int gsm610_write_block(SF_PRIVATE *psf, GSM610_PRIVATE *pgsm610,
    ...
    while (indx < len) {
        ...
        memcpy(&(pgsm610->samples[pgsm610->samplecount]), &(ptr[indx]),
                        count * sizeof(short));
        indx += count;
        pgsm610->samplecount += count;
        total = indx;
