...
static sf_count_t dwvw_write_s(SF_PRIVATE *psf, const short *ptr,
                               sf_count_t len);
static sf_count_t dwvw_write_i(SF_PRIVATE *psf, const int *ptr, sf_count_t len);
static sf_count_t dwvw_write_f(SF_PRIVATE *psf, const float *ptr,
                               sf_count_t len);
static sf_count_t dwvw_write_d(SF_PRIVATE *psf, const double *ptr,
                               sf_count_t len);

static sf_count_t dwvw_seek(SF_PRIVATE *psf, int mode, sf_count_t offset);
static int dwvw_close(SF_PRIVATE *psf);

static int dwvw_decode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw, int *ptr,
                            int len);
static int dwvw_decode_load_bits(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
                                 int bit_count);

static int dwvw_encode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
                            const int *ptr, int len);
static void dwvw_encode_store_bits(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
                                   int data, int new_bits);
...
static sf_count_t dwvw_seek(SF_PRIVATE *psf, int UNUSED (mode),
                sf_count_t offset) {
    DWVW_PRIVATE *pdwvw;
...
static int dwvw_decode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw, int *ptr,
    ...
    for (count = 0; count < len; count++) { /* If bit_count parameter is zero get the delta_width_modifier. */
        ...
        delta_width = (delta_width + delta_width_modifier + pdwvw->bit_width)
                % pdwvw->bit_width;

        /* Load the delta. */
        delta = 0;
        if (delta_width) {
            delta = dwvw_decode_load_bits(psf, pdwvw, delta_width - 1)
                    | (1 << (delta_width - 1));
...
static int dwvw_decode_load_bits(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
    ...
    if (!get_dwm) {
        output = (pdwvw->bits >> (pdwvw->bit_count - bit_count))
                & ((1 << bit_count) - 1);
...
#if 0
/* Debigging routine. */
static void dump_bits (DWVW_PRIVATE *pdwvw) {
    int k, mask;
...
static int dwvw_encode_data(SF_PRIVATE *psf, DWVW_PRIVATE *pdwvw,
    ...
    delta_width_modifier = (delta_width - pdwvw->last_delta_width)
            % pdwvw->bit_width;
