...
static void Calculation_of_the_LTP_parameters(register word * din, /* [0..39] IN */
                register word * dp, /* [-120..-1] IN */
                word * bc_out, /* OUT */
                word * Nc_out ) { /*  OUT */
    register int k, lambda;
    ...
    for (lambda = 40; lambda <= 120; lambda += 9) {
        ...
        register float W;
        register float a = lp[-8], b = lp[-7], c = lp[-6], d = lp[-5], e =
                lp[-4], f = lp[-3], g = lp[-2], h = lp[-1];
        register float E;
...
static void Long_term_analysis_filtering(word bc, /* IN  */
                word Nc, /* IN */
...
void Gsm_Long_Term_Synthesis_Filtering(struct gsm_state * S,
                word Ncr,
                word bcr, register word * erp, /* [0..39] IN */
