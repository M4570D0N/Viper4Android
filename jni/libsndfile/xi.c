...
int xi_open(SF_PRIVATE *psf) {
    ...
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR) {
        ...
        memcpy(pxi->filename, "Default Name            ",
                sizeof(pxi->filename));
        memcpy(pxi->software, "ViPER4Android           ",
                sizeof(pxi->software));
...
static sf_count_t dpcm_write_s2dsc(SF_PRIVATE *psf, const short *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_i2dsc(SF_PRIVATE *psf, const int *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_f2dsc(SF_PRIVATE *psf, const float *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_d2dsc(SF_PRIVATE *psf, const double *ptr,
        sf_count_t len);
...
static sf_count_t dpcm_write_s2dles(SF_PRIVATE *psf, const short *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_i2dles(SF_PRIVATE *psf, const int *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_f2dles(SF_PRIVATE *psf, const float *ptr,
        sf_count_t len);
static sf_count_t dpcm_write_d2dles(SF_PRIVATE *psf, const double *ptr,
        sf_count_t len);
...
static sf_count_t dpcm_seek(SF_PRIVATE *psf, int mode, sf_count_t offset) {
    ...
    if (mode != SFM_READ) { /* What to do about write??? */
        psf->error = SFE_BAD_SEEK;
        return PSF_SEEK_ERROR;
    };
...
static int xi_write_header(SF_PRIVATE *psf, int UNUSED (calc_length)) {
    ...
    psf_binheader_writef(psf, "eb2", pxi->software, sizeof(pxi->software),
                    (1 << 8) + 2);
    ...
    psf_binheader_writef(psf, "et844", psf->sf.frames, pxi->loop_begin,
                    pxi->loop_end);

    /* volume, fine tune, flags, pan, note, namelen */
    psf_binheader_writef(psf, "111111", 128, 0, pxi->sample_flags, 128, 0,
                    strlen(pxi->sample_name));
...
static void dsc2f_array(XI_PRIVATE *pxi, signed char *src,
                int count, float *dest, float normfact);
static void dsc2d_array(XI_PRIVATE *pxi, signed char *src, int count,
                double *dest, double normfact);
...
static void dles2f_array(XI_PRIVATE *pxi, short *src,
                int count, float *dest, float normfact);
static void dles2d_array(XI_PRIVATE *pxi, short *src,
                int count, double *dest, double normfact);
...
static void f2dles_array(XI_PRIVATE *pxi, const float *src, 
                short *dest, int count, float normfact);
static void d2dles_array(XI_PRIVATE *pxi, const double *src, 
                short *dest, int count, double normfact);
...
static void dsc2s_array(XI_PRIVATE *pxi, signed char *src,
                int count, short *dest) {
...
static void dsc2f_array(XI_PRIVATE *pxi, signed char *src, int count,
                float *dest, float normfact) {
    signed char last_val;
    int k;
    last_val = pxi->last_16 >> 8;
...
static void dsc2d_array(XI_PRIVATE *pxi, signed char *src,
                int count, double *dest, double normfact) {
    signed char last_val;
    int k;
    last_val = pxi->last_16 >> 8;
