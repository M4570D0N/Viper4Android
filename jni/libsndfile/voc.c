...
static int voc_read_header(SF_PRIVATE *psf) {
    ...
    offset = psf_binheader_readf(psf, "pb", 0, creative,
    ...
    offset += psf_binheader_readf(psf, "e222", &dataoffset, &version,
                    &checksum);
    ...
    if (block_type == VOC_SOUND_DATA) {
        ...
        offset += psf_binheader_readf(psf, "e311", &size, &rate_byte,
                        &compression);
    ...
    if (block_type == VOC_EXTENDED) {
        ...
        offset += psf_binheader_readf(psf, "e3211", &size, &rate_short, &pack,
                        &stereo);
    ...
    if (block_type == VOC_EXTENDED_II) {
        ...
        offset += psf_binheader_readf(psf, "e341124", &size,
                        &psf->sf.samplerate, &bitwidth, &channels, &encoding,
                        &fourbytes);
        ...
        if (size + 31 == psf->filelength + 1) { /* Hack for reading files produced using
...
static int voc_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "eb1", "Creative Voice File", make_size_t(19),
                    0x1A);
    ...
    if (subformat == SF_FORMAT_PCM_U8 && psf->sf.channels == 1) { /* samplerate = 1000000 / (256 - rate_const) ; */
        rate_const = 256 - 1000000 / psf->sf.samplerate;

        /* First type marker, length, rate_const and compression */
        psf_binheader_writef(psf, "e1311", VOC_SOUND_DATA,
                        (int) (psf->datalength + 1), rate_const, 0);
    } else if (subformat == SF_FORMAT_PCM_U8 && psf->sf.channels == 2) { /* sample_rate = 128000000 / (65536 - rate_short) ; */
        ...
        switch (subformat) {
            case SF_FORMAT_PCM_U8:
                ...
                psf_binheader_writef(psf, "e1341124", VOC_EXTENDED_II, length,
                                psf->sf.samplerate, 16, psf->sf.channels, 4, 0);
                break;
            ...
            case SF_FORMAT_ALAW:
                ...
                length = psf->sf.frames * psf->sf.channels * psf->bytewidth + 12;
                psf_binheader_writef(psf, "e1341124", VOC_EXTENDED_II, length,
                                psf->sf.samplerate, 8, psf->sf.channels, 6, 0);
                break;
            ...
            case SF_FORMAT_ULAW:
                ...
                length = psf->sf.frames * psf->sf.channels * psf->bytewidth + 12;
                psf_binheader_writef(psf, "e1341124", VOC_EXTENDED_II, length,
                                psf->sf.samplerate, 8, psf->sf.channels, 7, 0);
                break;
...
static int voc_close(SF_PRIVATE *psf) {
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR) { /*  Now we know for certain the length of the file we can re-write
