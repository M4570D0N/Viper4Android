...
static sf_count_t msadpcm_read_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms,
            short *ptr, int len);

static int msadpcm_encode_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms);
static sf_count_t msadpcm_write_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms,
...
static sf_count_t msadpcm_write_s(SF_PRIVATE *psf, const short *ptr,
            sf_count_t len);
static sf_count_t msadpcm_write_i(SF_PRIVATE *psf, const int *ptr,
            sf_count_t len);
static sf_count_t msadpcm_write_f(SF_PRIVATE *psf, const float *ptr,
            sf_count_t len);
static sf_count_t msadpcm_write_d(SF_PRIVATE *psf, const double *ptr,
            sf_count_t len);

static sf_count_t msadpcm_seek(SF_PRIVATE *psf, int mode, sf_count_t offset);
static int msadpcm_close(SF_PRIVATE *psf);

static void choose_predictor(unsigned int channels, short *data, int *bpred,
            int *idelta);
...
static int msadpcm_decode_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms) {
    ...
    for (k = 2 * pms->channels; k < (pms->samplesperblock * pms->channels); k++) {
        ...
        predict = ((pms->samples[k - pms->channels] * AdaptCoeff1[bpred[chan]])
                        + (pms->samples[k - 2 * pms->channels]
                                        * AdaptCoeff2[bpred[chan]])) >> 8; /* => / 256 => FIXED_POINT_COEFF_BASE == 256 */
...
static sf_count_t msadpcm_read_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms,
    ...
    while (indx < len) {
        ...
        memcpy(&(ptr[indx]), &(pms->samples[pms->samplecount * pms->channels]),
                        count * sizeof(short));
        indx += count;
...
static int msadpcm_encode_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms) {
    ...
    if (pms->channels == 1) {
        ...
        for (k = 2; k < pms->samplesperblock; k++) {
            predict = (pms->samples[k - 1] * AdaptCoeff1[bpred[0]]
                            + pms->samples[k - 2] * AdaptCoeff2[bpred[0]]) >> 8;
        ...
        for (k = 4; k < 2 * pms->samplesperblock; k++) {
            ...
            predict = (pms->samples[k - 2] * AdaptCoeff1[bpred[chan]]
                             + pms->samples[k - 4] * AdaptCoeff2[bpred[chan]]) >> 8;
...
static sf_count_t msadpcm_write_block(SF_PRIVATE *psf, MSADPCM_PRIVATE *pms,
    ...
    while (indx < len) {
        ...
        memcpy(&(pms->samples[pms->samplecount * pms->channels]), &(ptr[total]),
                        count * sizeof(short));
        indx += count;
        pms->samplecount += count / pms->channels;
        total = indx;
...
static int msadpcm_close(SF_PRIVATE *psf) {
    ...
    if (psf->file.mode == SFM_WRITE) { /*  Now we know static int for certain the length of the file we can
     **  re-write the header.
     */
...
static void choose_predictor(unsigned int channels, short *data,
    ...
    for (chan = 0; chan < channels; chan++) {
        ...
        for (bpred = 0; bpred < 7; bpred++) {
            ...
            for (k = 2; k < 2 + IDELTA_COUNT; k++)
                idelta_sum += abs(data[k * channels] - ((data[(k - 1) * channels]
                            * AdaptCoeff1[bpred] + data[(k - 2) * channels]* AdaptCoeff2[bpred]) >> 8));
