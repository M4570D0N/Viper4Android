 ...
 **   double | none none    none    none    none    none
 */
...
static sf_count_t dither_read_short(SF_PRIVATE * UNUSED (psf),
                short * UNUSED (ptr), sf_count_t len) {
    return len;
} /* dither_read_short */
...
static sf_count_t dither_write_short(SF_PRIVATE *psf, const short *ptr,
    ...
    while (len > 0) {
        ...
        thiswrite = pdither->write_short(psf, (short*) pdither->buffer,
                        writecount);
        total += thiswrite;
        len -= thiswrite;
...
static sf_count_t dither_write_float(SF_PRIVATE *psf, const float *ptr,
    ...
    while (len > 0) {
        ...
        thiswrite = pdither->write_float(psf, (float*) pdither->buffer,
                        writecount);
        total += thiswrite;
        len -= thiswrite;
        if (thiswrite < writecount)
...
static sf_count_t dither_write_double(SF_PRIVATE *psf, const double *ptr,
    ...
    while (len > 0) {
        ...
        thiswrite = pdither->write_double(psf, (double*) pdither->buffer,
                        writecount);
        total += thiswrite;
        len -= thiswrite;
        if (thiswrite < writecount)
...
int DO_NOT_USE_sf_dither_short (const SF_DITHER_INFO *dither,
                const short *in, short *out, int frames, int channels) {
    int ch, k;
...
int DO_NOT_USE_sf_dither_int (const SF_DITHER_INFO *dither,
                const int *in, int *out, int frames, int channels) {
    int ch, k;
...
int DO_NOT_USE_sf_dither_float (const SF_DITHER_INFO *dither,
                const float *in, float *out, int frames, int channels) {
    int ch, k;
...
int DO_NOT_USE_sf_dither_double (const SF_DITHER_INFO *dither,
                const double *in, double *out, int frames, int channels) {
    int ch, k;
