...
static int sd2_close(SF_PRIVATE *psf) {
    if (psf->file.mode == SFM_WRITE) { /*  Now we know for certain the audio_length of the file we can re-write
...
static inline void write_char(unsigned char * data, int offset, char value) {
    data[offset] = value;
} /* write_char */

static inline void write_short(unsigned char * data, int offset, short value) {
    data[offset] = value >> 8;
    data[offset + 1] = value;
} /* write_char */

static inline void write_int(unsigned char * data, int offset, int value) {
...
static inline void write_marker(unsigned char * data, int offset, int value) {
...
static void write_str(unsigned char * data, int offset, const char * buffer,
                int buffer_len) {
    memcpy(data + offset, buffer, buffer_len);
} /* write_str */

static int sd2_write_rsrc_fork(SF_PRIVATE *psf, int UNUSED (calc_length)) {
    SD2_RSRC rsrc;
    STR_RSRC str_rsrc[] = {
    ...
    write_short(rsrc.rsrc_data, rsrc.map_offset + 24,
                    rsrc.type_offset - rsrc.map_offset - 2);
    ...
    rsrc.string_offset = rsrc.item_offset + (rsrc.str_count + 1) * 12
                    - rsrc.map_offset;
    ...
    for (k = 0; k < ARRAY_LEN (str_rsrc) ; k++) {
        write_str(rsrc.rsrc_data, str_offset, str_rsrc[k].name,
                        strlen(str_rsrc[k].name));
        ...
        write_int(rsrc.rsrc_data, rsrc.item_offset + k * 12 + 4,
                        data_offset - rsrc.data_offset);

        write_int(rsrc.rsrc_data, data_offset, str_rsrc[k].value_len);
        write_str(rsrc.rsrc_data, data_offset + 4, str_rsrc[k].value,
...
static inline int read_char(const unsigned char * data, int offset) {
    return data[offset];
} /* read_char */

static inline int read_short(const unsigned char * data, int offset) {
    return (data[offset] << 8) + data[offset + 1];
} /* read_short */

static inline int read_int(const unsigned char * data, int offset) {
    return (data[offset] << 24) + (data[offset + 1] << 16)
                    + (data[offset + 2] << 8) + data[offset + 3];
} /* read_int */

static inline int read_marker(const unsigned char * data, int offset) {
    if (CPU_IS_BIG_ENDIAN)
        return (data[offset] << 24) + (data[offset + 1] << 16)
                        + (data[offset + 2] << 8) + data[offset + 3];
...
static void read_str(const unsigned char * data, int offset, char * buffer,
                int buffer_len) {
    int k;
...
static int sd2_parse_rsrc_fork(SF_PRIVATE *psf) {
    ...
    rsrc.string_offset = rsrc.map_offset
                    + read_short(rsrc.rsrc_data, rsrc.map_offset + 26);
...
static int parse_str_rsrc(SF_PRIVATE *psf, SD2_RSRC * rsrc) {
    ...
    for (k = 0; data_offset + data_len < rsrc->rsrc_len; k++) {
        ...
        read_str(rsrc->rsrc_data, str_offset + 1, name,
                        SF_MIN (SIGNED_SIZEOF (name), slen + 1));
        str_offset += slen + 1;

        rsrc_id = read_short(rsrc->rsrc_data, rsrc->item_offset + k * 12);

        data_offset = rsrc->data_offset + read_int(rsrc->rsrc_data,
                    rsrc->item_offset + k * 12 + 4);
        ...
        read_str(rsrc->rsrc_data, data_offset + 5, value,
