...
enum {
    HAVE_FORM = 0x01,
    HAVE_SVX = 0x02, HAVE_VHDR = 0x04, HAVE_BODY = 0x08
};
    ...
    while (!done) {
        ...
        switch (marker) {
            ...
            case VHDR_MARKER:
                ...
                psf_binheader_readf(psf, "E4442114", &(vhdr.oneShotHiSamples),
                                &(vhdr.repeatHiSamples), &(vhdr.samplesPerHiCycle),
                                &(vhdr.samplesPerSec), &(vhdr.octave),
                ...
                psf_log_printf(psf, "  Compression       : %d => ",
                                vhdr.compression);
...
static int svx_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "Etm8", FORM_MARKER,
                    (psf->filelength < 8) ?
    ...
    psf_binheader_writef(psf, "Emsms", NAME_MARKER, psf->file.name.c,
                    ANNO_MARKER, annotation);

    /* BODY marker and size. */
    psf_binheader_writef(psf, "Etm8", BODY_MARKER,
                    (psf->datalength < 0) ? psf->datalength * 0 : psf->datalength);
