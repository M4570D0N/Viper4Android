...
static int wavex_guid_equal(const EXT_SUBFORMAT * first,
                const EXT_SUBFORMAT * second) {
...
int wav_w64_read_fmt_chunk(SF_PRIVATE *psf, int fmtsize) {
    ...
    bytesread = psf_binheader_readf(psf, "224422", &(wav_fmt->format),
                    &(wav_fmt->min.channels), &(wav_fmt->min.samplerate),
                    &(wav_fmt->min.bytespersec), &(wav_fmt->min.blockalign),
                    &(wav_fmt->min.bitwidth));

    psf_log_printf(psf, "  Format        : 0x%X => %s\n", wav_fmt->format,
                    wav_w64_format_str(wav_fmt->format));
    ...
    if (wav_fmt->format == WAVE_FORMAT_PCM && wav_fmt->min.blockalign == 0
        ...
        wav_fmt->min.blockalign = wav_fmt->min.bitwidth / 8
                        + (wav_fmt->min.bitwidth % 8 > 0 ? 1 : 0);
    ...
    switch (wav_fmt->format) {
        ...
        case WAVE_FORMAT_MS_ADPCM:
            ...
            bytesread += psf_binheader_readf(psf, "222",
            ...
            for (k = 0; k < wav_fmt->msadpcm.numcoeffs; k++) {
                bytesread += psf_binheader_readf(psf, "22",
                                &(wav_fmt->msadpcm.coeffs[k].coeff1),
                                &(wav_fmt->msadpcm.coeffs[k].coeff2));
                snprintf(psf->u.cbuf, sizeof(psf->u.cbuf),
                                "     %2d     %7d   %7d\n", k,
                                wav_fmt->msadpcm.coeffs[k].coeff1,
                                wav_fmt->msadpcm.coeffs[k].coeff2);
        ...
        case WAVE_FORMAT_EXTENSIBLE:
            if (wav_fmt->ext.bytespersec / wav_fmt->ext.blockalign
                            != wav_fmt->ext.samplerate)
                psf_log_printf(psf, "  Bytes/sec     : %d (should be %d)\n",
            ...
            if (wav_fmt->ext.channelmask == 0)
                ...
                for (bit = k = 0; bit < ARRAY_LEN (channel_mask_bits) ; bit++) {
                    if (wav_fmt->ext.channelmask & (1 << bit)) {
                        ...
                        append_snprintf(psf->u.cbuf, sizeof(psf->u.cbuf), "%s, ",
            ...
            bytesread += psf_binheader_readf(psf, "422",
            ...
            for (k = 0; k < 8; k++) {
                bytesread += psf_binheader_readf(psf, "1",
                                &(wav_fmt->ext.esf.esf_field4[k]));
        ...
        case WAVE_FORMAT_G721_ADPCM:
            ...
            if (fmtsize >= 20) {
                bytesread += psf_binheader_readf(psf, "22",
                                &(wav_fmt->g72x.extrabytes), &(wav_fmt->g72x.auxblocksize));
...
void wavex_write_guid(SF_PRIVATE *psf, const EXT_SUBFORMAT * subformat) {
    psf_binheader_writef(psf, "422b", subformat->esf_field1, subformat->esf_field2,
                    subformat->esf_field3, subformat->esf_field4, make_size_t(8));
} /* wavex_write_guid */
