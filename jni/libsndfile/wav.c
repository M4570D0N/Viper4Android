...
static const EXT_SUBFORMAT MSGUID_SUBTYPE_ALAW = {
    0x00000006, 0x0000, 0x0010, {
        0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71
    }
};
static const EXT_SUBFORMAT MSGUID_SUBTYPE_MULAW = {
    0x00000007, 0x0000, 0x0010, {
        0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71
    }
};
...
static int wav_read_header(SF_PRIVATE *psf, int *blockalign, int *framesperblock) {
    ...
    while (!done) {
        ...
        switch (marker) {
                ...
                if (psf->fileoffset > 0 && psf->filelength > RIFFsize + 8) {
                    ...
                    if (marker == RIFF_MARKER)
                        psf_log_printf(psf, "RIFF : %u (should be %D)\n", RIFFsize,
                                        psf->filelength - 2 * SIGNED_SIZEOF (dword));
                    else
            ...
            case fmt_MARKER:
                if ((parsestage & (HAVE_RIFF | HAVE_WAVE))
                                != (HAVE_RIFF | HAVE_WAVE))
            ...
            case fact_MARKER:
                ...
                if (dword > SIGNED_SIZEOF (fact_chunk) )
                    psf_binheader_readf(psf, "j",
                                    (int) (dword - SIGNED_SIZEOF (fact_chunk) ));
            ...
            case PEAK_MARKER:
                ...
                psf_binheader_readf(psf, "44", &(psf->peak_info->version),
                ...
                psf->peak_info->peak_loc = ((parsestage & HAVE_data) == 0) ?
            ...
            case cue_MARKER:
                parsestage |= HAVE_other; {
                    ...
                    while (cue_count) {
                        bytesread += psf_binheader_readf(psf, "444444", &id,
                                        &position, &chunk_id, &chunk_start,
                                        &block_start, &offset);
                        psf_log_printf(psf, "   Cue ID : %2d"
                                        "  Pos : %5u  Chunk : %M"
                                        "  Chk Start : %d  Blk Start : %d"
                                        "  Offset : %5d\n", id, position, chunk_id,
    ...
    switch (format) {
        ...
        case WAVE_FORMAT_IEEE_FLOAT:
            ...
            psf->sf.format |= (psf->bytewidth == 8) ?
                            SF_FORMAT_DOUBLE : SF_FORMAT_FLOAT;
            break;
...
static int wav_write_fmt_chunk(SF_PRIVATE *psf) {
    ...
    switch (subformat) {
        ...
        case SF_FORMAT_PCM_32:
            ...
            psf_binheader_writef(psf, "4224", fmt_size, WAVE_FORMAT_PCM,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
            /*  fmt : blockalign, bitwidth */
            psf_binheader_writef(psf, "22", psf->bytewidth * psf->sf.channels,
                            psf->bytewidth * 8);
            break;
        ...
        case SF_FORMAT_DOUBLE:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "4224", fmt_size, WAVE_FORMAT_IEEE_FLOAT,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
        ...
        case SF_FORMAT_ULAW:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "4224", fmt_size, WAVE_FORMAT_MULAW,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
            /*  fmt : blockalign, bitwidth, extrabytes */
            psf_binheader_writef(psf, "222", psf->bytewidth * psf->sf.channels, 8,
                            0);
        ...
        case SF_FORMAT_ALAW:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "4224", fmt_size, WAVE_FORMAT_ALAW,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
        ...
        case SF_FORMAT_IMA_ADPCM: {
            ...
            blockalign = wav_w64_srate2blocksize(
                            psf->sf.samplerate * psf->sf.channels);
            framesperblock = 2 * (blockalign - 4 * psf->sf.channels)
                            / psf->sf.channels + 1;
        ...
        case SF_FORMAT_MS_ADPCM: {
            ...
            blockalign = wav_w64_srate2blocksize(
                            psf->sf.samplerate * psf->sf.channels);
            framesperblock = 2
                            + 2 * (blockalign - 7 * psf->sf.channels)
                                            / psf->sf.channels;
            bytespersec = (psf->sf.samplerate * blockalign) / framesperblock;
            ...
            psf_binheader_writef(psf, "422", fmt_size, WAVE_FORMAT_MS_ADPCM,
                            psf->sf.channels);
            ...
            /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
            psf_binheader_writef(psf, "22222", blockalign, 4, extrabytes,
                            framesperblock, 7);
        ...
        case SF_FORMAT_GSM610: {
            ...
            fmt_size = 2 + 2 + 4 + 4 + 2 + 2 + 2 + 2; /* fmt chunk. */
            psf_binheader_writef(psf, "422", fmt_size, WAVE_FORMAT_GSM610,
                            psf->sf.channels); /* fmt : size, WAV format type, channels. */
            psf_binheader_writef(psf, "44", psf->sf.samplerate, bytespersec); /* fmt : samplerate, bytespersec. */
            psf_binheader_writef(psf, "2222", blockalign, 0, 2, framesperblock); /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
        }
            ;
...
static int wavex_write_fmt_chunk(SF_PRIVATE *psf) {
    ...
    switch (subformat) {
        ...
        case SF_FORMAT_ALAW:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "4224", fmt_size, WAVE_FORMAT_EXTENSIBLE,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
...
static int wav_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    if (psf->endian == SF_ENDIAN_LITTLE)
        psf_binheader_writef(psf, "etm8", RIFF_MARKER,
                        (psf->filelength < 8) ? 8 : psf->filelength - 8);
    else
        psf_binheader_writef(psf, "Etm8", RIFX_MARKER,
                        (psf->filelength < 8) ? 8 : psf->filelength - 8);
    ...
    if (psf->peak_info != NULL && psf->peak_info->peak_loc == SF_PEAK_START) {
        ...
        for (k = 0; k < psf->sf.channels; k++)
            psf_binheader_writef(psf, "ft8", (float) psf->peak_info->peaks[k].value,
    ...
    if (psf->instrument != NULL) {
        ...
        for (tmp = 0; tmp < psf->instrument->loop_count; tmp++) {
            ...
            type = (type == SF_LOOP_FORWARD ? 0 : type == SF_LOOP_BACKWARD ? 2 :
                    type == SF_LOOP_ALTERNATING ? 1 : 32);
            psf_binheader_writef(psf, "44", tmp, type);
            psf_binheader_writef(psf, "44", psf->instrument->loops[tmp].start,
                            psf->instrument->loops[tmp].end - 1);
            psf_binheader_writef(psf, "44", 0,
                            psf->instrument->loops[tmp].count);
        };
    };
...
static int wav_write_tailer(SF_PRIVATE *psf) {
    ...
    if (psf->peak_info != NULL && psf->peak_info->peak_loc == SF_PEAK_END) {
        psf_binheader_writef(psf, "m4", PEAK_MARKER,
                        WAV_PEAK_CHUNK_SIZE (psf->sf.channels));
        psf_binheader_writef(psf, "44", 1, time(NULL));
        for (k = 0; k < psf->sf.channels; k++)
            psf_binheader_writef(psf, "f4", psf->peak_info->peaks[k].value,
                            psf->peak_info->peaks[k].position);
    };
...
static int wav_command(SF_PRIVATE *psf, int command, void * UNUSED (data),
    ...
    switch (command) {
        ...
        case SFC_SET_CHANNEL_MAP_INFO:
            wpriv->wavex_channelmask = wavex_gen_channel_mask(psf->channel_map,
                            psf->sf.channels);
    ...
    while (bytesread < length) {
        ...
        switch (chunk) {
            ...
            case 0:
                ...
                if (bytesread < length)
                    bytesread += psf_binheader_readf(psf, "j",
                                    length - bytesread + 4);
...
static int wav_read_acid_chunk(SF_PRIVATE *psf, unsigned int chunklen) {
    ...
    bytesread += psf_binheader_readf(psf, "422f", &beats, &meter_denom,
                    &meter_numer, &tempo);
...
int wav_read_bext_chunk(SF_PRIVATE *psf, unsigned int chunksize) {
    ...
    b = psf->broadcast_16k;
    bytes += psf_binheader_readf(psf, "b", b->description,
                    sizeof(b->description));
    bytes += psf_binheader_readf(psf, "b", b->originator,
                    sizeof(b->originator));
    bytes += psf_binheader_readf(psf, "b", b->originator_reference,
                    sizeof(b->originator_reference));
    bytes += psf_binheader_readf(psf, "b", b->origination_date,
                    sizeof(b->origination_date));
    bytes += psf_binheader_readf(psf, "b", b->origination_time,
...
int wav_write_bext_chunk(SF_PRIVATE *psf) {
    ...
    b = psf->broadcast_16k;
    psf_binheader_writef(psf, "m4", bext_MARKER,
                    WAV_BEXT_MIN_CHUNK_SIZE + b->coding_history_size);
    ...
    psf_binheader_writef(psf, "b", b->originator_reference,
                    sizeof(b->originator_reference));
    psf_binheader_writef(psf, "b", b->origination_date,
                    sizeof(b->origination_date));
    psf_binheader_writef(psf, "b", b->origination_time,
                    sizeof(b->origination_time));
    psf_binheader_writef(psf, "442", b->time_reference_low,
                    b->time_reference_high, b->version);
    psf_binheader_writef(psf, "b", b->umid, sizeof(b->umid));
    psf_binheader_writef(psf, "z", make_size_t(190));

    if (b->coding_history_size > 0)
        psf_binheader_writef(psf, "b", b->coding_history,
                        make_size_t(b->coding_history_size));

    return 0;
} /* wav_write_bext_chunk */

static int exif_fill_and_sink(SF_PRIVATE *psf, char* buf, size_t bufsz,
                size_t toread) {
    size_t bytesread = 0;
    ...
    while (bytesread < length) {
        ...
        switch (marker) {
            ...
            case ever_MARKER:
                ...
                vmajor = 10 * (((dword >> 24) & 0xff) - '0')
                                + (((dword >> 16) & 0xff) - '0');
                vminor = 10 * (((dword >> 8) & 0xff) - '0')
                                + ((dword & 0xff) - '0');
