...
static sf_count_t paf24_write_s(SF_PRIVATE *psf, const short *ptr,
            sf_count_t len);
static sf_count_t paf24_write_i(SF_PRIVATE *psf, const int *ptr, sf_count_t len);
static sf_count_t paf24_write_f(SF_PRIVATE *psf, const float *ptr,
            sf_count_t len);
static sf_count_t paf24_write_d(SF_PRIVATE *psf, const double *ptr,
            sf_count_t len);
...
int paf_open(SF_PRIVATE *psf) {
    ...
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR) {
        ...
        if (endian == SF_ENDIAN_LITTLE
                        || (CPU_IS_LITTLE_ENDIAN && (endian == SF_ENDIAN_CPU)))
...
static int paf_write_header(SF_PRIVATE *psf, int UNUSED (calc_length)) {
    ...
    if (psf->endian == SF_ENDIAN_BIG) { /* Marker, version, endianness, samplerate */
    ...
    } else if (psf->endian == SF_ENDIAN_LITTLE) { /* Marker, version, endianness, samplerate */
...
static int paf24_init(SF_PRIVATE *psf) {
    ...
    ppaf24->block = (unsigned char*) (ppaf24->data
                    + PAF24_SAMPLES_PER_BLOCK * ppaf24->channels);
...
static int paf24_read_block(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24) {
    ...
    /* Read the block. */
    if ((k = psf_fread(ppaf24->block, 1, ppaf24->blocksize, psf))
                    != ppaf24->blocksize)
    ...
    } else { /* Do endian swapping if necessary. */
        if (psf->endian == SF_ENDIAN_BIG)
            endswap_int_array(ppaf24->data, 8 * ppaf24->channels);
...
static int paf24_write_block(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24) {
    ...
    if (CPU_IS_LITTLE_ENDIAN) {
        ...
        if (psf->endian == SF_ENDIAN_BIG)
            endswap_int_array(ppaf24->data, 8 * ppaf24->channels);
    } else if (CPU_IS_BIG_ENDIAN) { /* This is correct. */
        for (k = 0; k < PAF24_SAMPLES_PER_BLOCK * ppaf24->channels; k++) {
            channel = k % ppaf24->channels;
            cptr = ppaf24->block + PAF24_BLOCK_SIZE * channel + 3 * (k / ppaf24->channels);
            nextsample = ppaf24->samples[k] >> 8;
    ...
    /* Write block to disk. */
    if ((k = psf_fwrite(ppaf24->block, 1, ppaf24->blocksize, psf))
                    != ppaf24->blocksize)
        psf_log_printf(psf, "*** Warning : short write (%d != %d).\n", k,
                        ppaf24->blocksize);

    if (ppaf24->sample_count
                    < ppaf24->write_block * ppaf24->samplesperblock + ppaf24->write_count)
        ppaf24->sample_count = ppaf24->write_block * ppaf24->samplesperblock
        + ppaf24->write_count;
...
static int paf24_write(SF_PRIVATE *psf, PAF24_PRIVATE *ppaf24, const int *ptr,
    ...
    while (total < len) {
        ...
        memcpy(&(ppaf24->samples[ppaf24->write_count * ppaf24->channels]),
                        &(ptr[total]), count * sizeof(int));
        total += count;
