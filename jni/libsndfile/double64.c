...
double double64_be_read(unsigned char *cptr) {
    ...
    upper = (((cptr[1] & 0xF) << 24) | (cptr[2] << 16) | (cptr[3] << 8)
            | cptr[4]);
...
double double64_le_read(unsigned char *cptr) {
    ...
    /* Might not have a 64 bit long, so load the mantissa into a double. */
    upper = ((cptr[6] & 0xF) << 24) | (cptr[5] << 16) | (cptr[4] << 8)
            | cptr[3];
...
static void double64_peak_update(SF_PRIVATE *psf, const double *buffer,
                                 int count, sf_count_t indx) {
    int chan;
    int k, position;
    float fmaxval;
    ...
    for (chan = 0; chan < psf->sf.channels; chan++) {
        ...
        if (fmaxval > psf->peak_info->peaks[chan].value) {
            ...
            psf->peak_info->peaks[chan].position = psf->write_current + indx
...
static int double64_get_capability(SF_PRIVATE *psf) {
    ...
    if (!psf->ieee_replace) { /* If this test is true ints and floats are compatible and little endian. */
        if (data.c[0] == 0xfb && data.c[1] == 0x59 && data.c[2] == 0x8c
                && data.c[3] == 0x42 && data.c[4] == 0xca && data.c[5] == 0xc0
                && data.c[6] == 0xf3 && data.c[7] == 0x3f)
            return DOUBLE_CAN_RW_LE;

        /* If this test is true ints and floats are compatible and big endian. */
        if (data.c[0] == 0x3f && data.c[1] == 0xf3 && data.c[2] == 0xc0
                && data.c[3] == 0xca && data.c[4] == 0x42 && data.c[5] == 0x8c
                && data.c[6] == 0x59 && data.c[7] == 0xfb)
...
static void d2s_array(const double *src, int count, short *dest, double scale) {
    while (--count >= 0) {
        dest[count] = lrint(scale * src[count]);
    };
} /* d2s_array */

static void d2s_clip_array(const double *src, int count, short *dest,
                           double scale) {
    while (--count >= 0) {
...
static void d2i_array(const double *src, int count, int *dest, double scale) {
    while (--count >= 0) {
        dest[count] = lrint(scale * src[count]);
    };
} /* d2i_array */

static void d2i_clip_array(const double *src, int count, int *dest,
                           double scale) {
    while (--count >= 0) {
...
static inline void d2f_array(const double *src, int count, float *dest) {
    while (--count >= 0) {
        dest[count] = src[count];
    };
} /* d2f_array */

static inline void s2d_array(const short *src, double *dest, int count,
                             double scale) {
    while (--count >= 0) {
...
static inline void i2d_array(const int *src, double *dest, int count,
                             double scale) {
    while (--count >= 0) {
        dest[count] = scale * src[count];
    };
...
static inline void f2d_array(const float *src, double *dest, int count) {
    while (--count >= 0) {
        dest[count] = src[count];
    };
} /* f2d_array */
...
static sf_count_t host_write_s2d(SF_PRIVATE *psf, const short *ptr,
                                 sf_count_t len) {
    ...
    while (len > 0) {
        ...
        if (psf->peak_info)
            double64_peak_update(psf, psf->u.dbuf, bufferlen,
                                 total / psf->sf.channels);
...
static sf_count_t host_write_i2d(SF_PRIVATE *psf, const int *ptr,
                                 sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
    double scale;
...
static sf_count_t host_write_f2d(SF_PRIVATE *psf, const float *ptr,
                                 sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
...
static sf_count_t host_write_d(SF_PRIVATE *psf, const double *ptr,
                               sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
...
static sf_count_t replace_write_s2d(SF_PRIVATE *psf, const short *ptr,
                                    sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
    double scale;
...
static sf_count_t replace_write_i2d(SF_PRIVATE *psf, const int *ptr,
                                    sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
    double scale;
...
static sf_count_t replace_write_f2d(SF_PRIVATE *psf, const float *ptr,
                                    sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
...
static sf_count_t replace_write_d(SF_PRIVATE *psf, const double *ptr,
                                  sf_count_t len) {
    int bufferlen, writecount;
    sf_count_t total = 0;
