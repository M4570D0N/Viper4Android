...
enum {
    basc_SCALE_MINOR = 1, basc_SCALE_MAJOR, basc_SCALE_NEITHER, basc_SCALE_BOTH
};

enum {
    basc_TYPE_LOOP = 0, basc_TYPE_ONE_SHOT
};
...
static unsigned int marker_to_position(const MARK_ID_POS *m, unsigned short n,
        int marksize);
...
int aiff_open(SF_PRIVATE *psf) {
    ...
    case SF_FORMAT_IMA_ADPCM:
         ...
         */
        error = aiff_ima_init(psf, AIFC_IMA4_BLOCK_LEN,
...
static int aiff_read_header(SF_PRIVATE *psf, COMM_CHUNK *comm_fmt) {
    ...
    while (!done) {
        ...
        case FORM_MARKER:
            ...
            if (psf->fileoffset > 0 && psf->filelength > FORMsize + 8) { /* Set file length. */
        ...
        case COMM_MARKER:
            ...
            pchk4_store(&paiff->chunk4, marker, paiff->comm_offset,
                    comm_fmt->size);
        ...
        case PEAK_MARKER:
            ...
            psf_binheader_readf(psf, "E44", &(psf->peak_info->version),
            ...
            psf->peak_info->peak_loc =
                    ((found_chunk & HAVE_SSND) == 0) ?
                            SF_PEAK_START : SF_PEAK_END;
            break;

        case SSND_MARKER:
            if ((found_chunk & HAVE_AIFC) && (found_chunk & HAVE_FVER) == 0)
                psf_log_printf(psf,
                        "*** Valid AIFC files should have an FVER chunk.\n");

            paiff->ssnd_offset = psf_ftell(psf) - 4;
            psf_binheader_readf(psf, "E444", &SSNDsize, &(ssnd_fmt.offset),
                    &(ssnd_fmt.blocksize));
            ...
            if (psf->datalength > psf->filelength - psf->dataoffset
                ...
                psf_log_printf(psf, " SSND : %u (should be %D)\n", SSNDsize,
                        psf->filelength - psf->dataoffset + sizeof(SSND_CHUNK));
                psf->datalength = psf->filelength - psf->dataoffset;
            } else
                psf_log_printf(psf, " SSND : %u\n", SSNDsize);

            if (ssnd_fmt.offset == 0
            ...
            psf_fseek(psf, psf->dataoffset + psf->datalength + (SSNDsize & 1),
                    SEEK_SET);
            break;
        ...
        case COMT_MARKER: {
            ...
            for (k = 0; k < count; k++) {
                dword -= psf_binheader_readf(psf, "E422", &timestamp, &id,
                        &len);
                psf_log_printf(psf,
                        "   time   : 0x%x\n   marker : %x\n   length : %d\n",
                        timestamp, id, len);
        ...
        case APPL_MARKER: {
            ...
            cptr = psf->u.cbuf;
            psf_binheader_readf(psf, "mb", &appl_marker, cptr,
                    dword + (dword & 1) - 4);
            cptr[dword] = 0;
        ...
        case INST_MARKER:
            ...
            psf_log_printf(psf, " %M : %d\n", marker, dword); {
                ...
                if (psf->instrument == NULL && (psf->instrument =
        ...
        case MARK_MARKER:
            ...
            psf_log_printf(psf, " %M : %d\n", marker, dword); {
                ...
                if (paiff->markstr != NULL) {
                    psf_log_printf(psf,
                            "*** Second MARK chunk found. Throwing away the first.\n");
                    free(paiff->markstr);
                };
                ...
                for (n = 0; n < mark_count && bytesread < dword; n++) {
                    ...
                    bytesread += psf_binheader_readf(psf, "E241", &mark_id,
                            &position, &ch);
                    ...
                    if (pstr_len < sizeof(psf->u.scbuf) - 1) {
                        ...
                        unsigned int read_len = pstr_len
                                - (sizeof(psf->u.scbuf) - 1);
                        bytesread += psf_binheader_readf(psf, "bj",
                                psf->u.scbuf, read_len, pstr_len - read_len);
        ...
        default:
            if (psf_isprint((marker >> 24) & 0xFF)
                    && psf_isprint((marker >> 16) & 0xFF)
                    && psf_isprint((marker >> 8) & 0xFF)
                    && psf_isprint(marker & 0xFF)) {
                psf_binheader_readf(psf, "E4", &dword);
            ...
            if ((dword = psf_ftell(psf)) & 0x03) {
                psf_log_printf(psf,
                        "  Unknown chunk marker %X at position %d. Resyncing.\n",
                        marker, dword - 4);
    ...
    if (instr_found && mark_found) {
        ...
        for (j = 0; j < psf->instrument->loop_count; j++) {
            if (j < ARRAY_LEN (psf->instrument->loops) ) {
                psf->instrument->loops[j].start = marker_to_position(
                        paiff->markstr, psf->instrument->loops[j].start,
                        mark_count);
                psf->instrument->loops[j].end = marker_to_position(
                        paiff->markstr, psf->instrument->loops[j].end,
                        mark_count);
...
static int aiff_read_comm_chunk(SF_PRIVATE *psf, COMM_CHUNK *comm_fmt) {
    ...
    bytesread += psf_binheader_readf(psf, "E242b", &(comm_fmt->numChannels),
            &(comm_fmt->numSampleFrames), &(comm_fmt->sampleSize),
            &(comm_fmt->sampleRate), SIGNED_SIZEOF (comm_fmt->sampleRate) );

    if (comm_fmt->size > 0x10000 && (comm_fmt->size & 0xffff) == 0) {
        psf_log_printf(psf, " COMM : %d (0x%x) *** should be ", comm_fmt->size,
                comm_fmt->size);
    ...
    if (comm_fmt->size == SIZEOF_AIFF_COMM)
        ...
        bytesread += psf_binheader_readf(psf, "Em1", &(comm_fmt->encoding),
                &encoding_len);

        comm_fmt->size =
                SF_MIN (sizeof (psf->u.scbuf), make_size_t (comm_fmt->size));
    ...
    psf_log_printf(psf, "  Frames      : %u%s\n", comm_fmt->numSampleFrames,
            (comm_fmt->numSampleFrames == 0 && psf->filelength > 104) ?
                    " (Should not be 0)" : "");
    ...
    } else if ((comm_fmt->encoding == fl64_MARKER
            || comm_fmt->encoding == FL64_MARKER)&& comm_fmt->sampleSize != 64){
...
static int aiff_rewrite_header(SF_PRIVATE *psf, AIFF_PRIVATE * paiff) {
    ...
    for (k = 0; k < paiff->chunk4.count; k++) {
        ...
        case COMM_MARKER:
            ...
            psf_binheader_writef(psf, "Em42t4", COMM_MARKER, comm_size,
                    psf->sf.channels, comm_frames);
            break;

        case SSND_MARKER:
            psf->headindex = paiff->chunk4.l[k].offset;
            psf_binheader_writef(psf, "Etm8", SSND_MARKER,
                    psf->datalength + SIZEOF_SSND_CHUNK);
            break;

        case PEAK_MARKER:
            psf->headindex = paiff->chunk4.l[k].offset;
            psf_binheader_writef(psf, "Em4", PEAK_MARKER,
                    AIFF_PEAK_CHUNK_SIZE (psf->sf.channels));
            psf_binheader_writef(psf, "E44", 1, time(NULL));
            for (ch = 0; ch < psf->sf.channels; ch++)
                psf_binheader_writef(psf, "Eft8",
                        (float) psf->peak_info->peaks[ch].value,
...
static int aiff_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    if (psf->file.mode == SFM_RDWR && psf->dataoffset > 0
            && paiff->chunk4.count > 0) {
    ...
    if (comm_type == AIFC_MARKER)
        ...
        psf_binheader_writef(psf, "Emm44", comm_type, FVER_MARKER, 4,
                0xA2805140);
    else
    ...
    psf_binheader_writef(psf, "Em42t42", COMM_MARKER, comm_size,
            psf->sf.channels, comm_frames, bit_width);
    psf_binheader_writef(psf, "b", comm_sample_rate, sizeof(comm_sample_rate));

    /* AIFC chunks have some extra data. */
    if (comm_type == AIFC_MARKER)
        psf_binheader_writef(psf, "mb", comm_encoding, comm_zero_bytes,
                sizeof(comm_zero_bytes));

    if (psf->channel_map && paiff->chanmap_tag)
        psf_binheader_writef(psf, "Em4444", CHAN_MARKER, 12, paiff->chanmap_tag,
                0, 0);
    ...
    if (psf->instrument != NULL) {
        ...
        psf_binheader_writef(psf, "Em4111111", INST_MARKER, SIZEOF_INST_CHUNK,
                ch.baseNote, ch.detune, ch.lowNote, ch.highNote, ch.lowVelocity,
                ch.highVelocity);
        psf_binheader_writef(psf, "2222222", ch.gain, ch.sustain_loop.playMode,
                ch.sustain_loop.beginLoop, ch.sustain_loop.endLoop,
                ch.release_loop.playMode, ch.release_loop.beginLoop,
                ch.release_loop.endLoop);

        if (ct == 2)
            psf_binheader_writef(psf, "Em42241b241b", MARK_MARKER,
                    2 + 2 * (2 + 4 + 1 + 9), 2, m[0].markerID, m[0].position, 8,
                    "beg loop", make_size_t(9), m[1].markerID, m[1].position, 8,
                    "end loop", make_size_t(9));
        else if (ct == 4)
            psf_binheader_writef(psf, "Em42 241b 241b 241b 241b", MARK_MARKER,
                    2 + 4 * (2 + 4 + 1 + 9), 4, m[0].markerID, m[0].position, 8,
                    "beg loop", make_size_t(9), m[1].markerID, m[1].position, 8,
                    "end loop", make_size_t(9), m[2].markerID, m[2].position, 8,
                    "beg loop", make_size_t(9), m[3].markerID, m[3].position, 8,
                    "end loop", make_size_t(9));
    };
    ...
    if (psf->peak_info != NULL && psf->peak_info->peak_loc == SF_PEAK_START) {
        ...
        for (k = 0; k < psf->sf.channels; k++)
            psf_binheader_writef(psf, "Eft8",
                    (float) psf->peak_info->peaks[k].value,
                    psf->peak_info->peaks[k].position);
    };

    /* Write SSND chunk. */
    paiff->ssnd_offset = psf->headindex;
    psf_binheader_writef(psf, "Etm844", SSND_MARKER,
            psf->datalength + SIZEOF_SSND_CHUNK, 0, 0);
...
static void aiff_write_strings(SF_PRIVATE *psf, int location) {
    ...
    for (k = 0; k < SF_MAX_STRINGS; k++) {
        ...
        case SF_STR_SOFTWARE:
            ...
            psf_binheader_writef(psf, "Em4mb", APPL_MARKER, slen + 4,
                    m3ga_MARKER, psf->strings[k].str,
                    make_size_t(slen + (slen & 1)));
            break;
...
static int aiff_command(SF_PRIVATE * psf, int command, void * UNUSED (data),
    ...
    case SFC_SET_CHANNEL_MAP_INFO:
        paiff->chanmap_tag = aiff_caf_find_channel_layout_tag(psf->channel_map,
                psf->sf.channels);
...
static int tenbytefloat2int(unsigned char *bytes) {
    ...
    val = (bytes[2] << 23) | (bytes[3] << 15) | (bytes[4] << 7)
            | (bytes[5] >> 1);
...
static int aiff_read_basc_chunk(SF_PRIVATE * psf, int datasize) {
    ...
    count = psf_binheader_readf(psf, "E442", &bc.version, &bc.numBeats,
            &bc.rootNote);
    count += psf_binheader_readf(psf, "E222", &bc.scaleType, &bc.sigNumerator,
            &bc.sigDenominator);
    count += psf_binheader_readf(psf, "E2j", &bc.loopType,
            datasize - sizeof(bc));
    ...
    psf->loop_info->loop_mode =
            (bc.loopType == basc_TYPE_ONE_SHOT) ?
                    SF_LOOP_NONE : SF_LOOP_FORWARD;
