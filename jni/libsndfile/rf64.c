 ...
 */
enum {
    HAVE_ds64 = 0x01, HAVE_fmt = 0x02, HAVE_bext = 0x04, HAVE_data = 0x08
};

#define HAVE_CHUNK(CHUNK)	((parsestage & CHUNK) != 0)

static int rf64_read_header(SF_PRIVATE *psf, int *blockalign,
                int *framesperblock) {
    WAV_PRIVATE *wpriv;
    WAV_FMT *wav_fmt;
    ...
    while (NOT (done)) {
        ...
        switch (marker) {
            case ds64_MARKER: {
                ...
                bytesread = psf_binheader_readf(psf, "888", &riff_size,
                                &ds64_datalength, &frame_count);
    ...
    switch (format) {
        case WAVE_FORMAT_EXTENSIBLE:
            ...
            psf->sf.format = (psf->sf.format & ~SF_FORMAT_TYPEMASK)
                            | SF_FORMAT_RF64;
...
static int rf64_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    /* Currently no table. */
    psf_binheader_writef(psf, "m48884", ds64_MARKER, 28, psf->filelength - 8,
                    psf->datalength, psf->sf.frames, 0);
