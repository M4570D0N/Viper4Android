...
static SF_FORMAT_INFO const simple_formats[] = {
    ...
    { SF_FORMAT_WAV | SF_FORMAT_PCM_U8, "WAV (Microsoft 8 bit PCM)",
                    "wav" },
...
static SF_FORMAT_INFO const major_formats [] = {
    ...
    #endif
    { SF_FORMAT_PAF,m"PAF (Ensoniq PARIS)", "paf"},
    ...
    { SF_FORMAT_IRCAM,m"SF (Berkeley/IRCAM/CARL)", "sf"},
...
int psf_get_format_info(SF_FORMAT_INFO *data) {
    ...
    if (SF_CONTAINER (data->format)) {
        ...
        for (k = 0;
                k < (SIGNED_SIZEOF (major_formats)
                        / SIGNED_SIZEOF (SF_FORMAT_INFO)); k++) {
        ...
        for (k = 0;
                k < (SIGNED_SIZEOF (subtype_formats)
                        / SIGNED_SIZEOF (SF_FORMAT_INFO) ); k++) {
