...
/*
 * psf_log_printf allows internal functions to print to an internal logbuffer which
    ...
    while ((c = *format++)) {
        ...
        case '8':
            ...
            if (psf->rwf_endian == SF_ENDIAN_BIG && trunc_8to4 == SF_FALSE) {
                ...
                count += 8;
            } else if (psf->rwf_endian == SF_ENDIAN_BIG
                    && trunc_8to4 == SF_TRUE) {
...
static int header_read(SF_PRIVATE *psf, void *ptr, int bytes) {
    ...
    if (psf->headindex + bytes > psf->headend) {
        count = psf_fread(psf->header + psf->headend, 1,
                bytes - (psf->headend - psf->headindex), psf);
...
static void header_seek(SF_PRIVATE *psf, sf_count_t position, int whence) {
    ...
    switch (whence) {
    case SEEK_SET:
        if (position > SIGNED_SIZEOF (psf->header) ) { /* Too much header to cache so just seek instead. */
            psf_fseek(psf, position, whence);
            return;
        };
        if (position > psf->headend)
            psf->headend += psf_fread(psf->header + psf->headend, 1,
                    position - psf->headend, psf);
    ...
    case SEEK_CUR:
        ...
        if (psf->headindex + position > SIGNED_SIZEOF (psf->header) ) { /* Need to jump this without caching it. */
        ...
        psf->headend += psf_fread(psf->header + psf->headend, 1,
                position - (psf->headend - psf->headindex), psf);
        psf->headindex = psf->headend;
        break;
...
int psf_binheader_readf(SF_PRIVATE *psf, char const *format, ...) {
    ...
    while ((c = *format++)) {
        ...
        case 'h':
            intptr = va_arg (argptr, unsigned int*);
            ucptr = (unsigned char*) intptr;
            byte_count += header_read(psf, sixteen_bytes,
...
sf_count_t psf_default_seek(SF_PRIVATE *psf, int UNUSED (mode),
