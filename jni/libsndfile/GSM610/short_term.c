...
static void Coefficients_0_12(register word * LARpp_j_1,
...
static void Coefficients_13_26(register word * LARpp_j_1,
                register word * LARpp_j, register word * LARp) {
    register int i;
...
static void Coefficients_27_39(register word * LARpp_j_1,
                register word * LARpp_j, register word * LARp) {
    register int i;
...
/* 4.2.9.2 */
static void LARp_to_rp(register word * LARp) { /* [0..7] IN/OUT  */
 ...
 */
    ...
    for (i = 1; i <= 8; i++, LARp++) {
        ...
        if (*LARp < 0) {
            ...
            *LARp = -((temp < 11059) ? temp << 1: ((temp < 20070) ? temp + 11059:
...
static void Fast_Short_term_analysis_filtering(struct gsm_state * S,
                register word * rp, /* [0..7] IN */
                register int k_n, /*   k_end - k_start */
                register word * s) { /* [0..n-1] IN/OUT */
        ...
        for (i = 8; i--;) {
            ...
            tmp2 =
                    (tmp1 == MIN_WORD && tmp2 == MIN_WORD ?
                            MAX_WORD: 0x0FFFF
                            & (((longword) tmp1 * (longword) tmp2 + 16384) >> 15));
             ...
             */
            tmp1 =
                    (tmp1 == MIN_WORD && sri == MIN_WORD ?
                            MAX_WORD: 0x0FFFF
                            & (((longword) tmp1 * (longword) sri + 16384) >> 15));
...
static void Fast_Short_term_synthesis_filtering(struct gsm_state * S,
                register word * rrp, /* [0..7] IN */
                register int k, /* k_end - k_start */
                register word * wt, /* [0..k-1] IN */
                register word * sr) { /* [0..k-1] OUT */
...
word * LARcr, /* received log area ratios [0..7] IN  */
    ...
    #undef FILTER
        ...
        #define FILTER (* (S->fast ? Fast_Short_term_synthesis_filtering: \
