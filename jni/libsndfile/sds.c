...
static sf_count_t sds_write_d(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
...
static int sds_read(SF_PRIVATE *psf, SDS_PRIVATE *psds, int *iptr,
                int readcount);
...
static int sds_write(SF_PRIVATE *psf, SDS_PRIVATE *psds, const int *iptr,
                int writecount);
...
static int sds_close(SF_PRIVATE *psf) {
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR) {
        ...
        if (psds->write_count > 0) {
            memset(&(psds->write_data[psds->write_count]), 0,
                            (psds->samplesperblock - psds->write_count)
                                            * sizeof(int));
...
static int sds_read_header(SF_PRIVATE *psf, SDS_PRIVATE *psds) {
    ...
    bytesread += psf_binheader_readf(psf, "e3331", &data_length,
...
static int sds_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "e33311", data_length, sustain_loop_start,
                    sustain_loop_end, loop_type, 0xF7);
...
static int sds_2byte_read(SF_PRIVATE *psf, SDS_PRIVATE *psds) {
    ...
    if ((k = psf_fread(psds->read_data, 1, SDS_BLOCK_SIZE, psf))
                    != SDS_BLOCK_SIZE)
    ...
    if (checksum != psds->read_data[SDS_BLOCK_SIZE - 2]) {
        psf_log_printf(psf, "Block %d : checksum is %02X should be %02X\n",
                        psds->read_data[4], checksum,
                        psds->read_data[SDS_BLOCK_SIZE - 2]);
    };
...
static int sds_read(SF_PRIVATE *psf, SDS_PRIVATE *psds, int *ptr, int len) {
    ...
    while (total < len) {
        ...
        memcpy(&(ptr[total]), &(psds->read_samples[psds->read_count]),
                        count * sizeof(int));
        total += count;
...
static int sds_write(SF_PRIVATE *psf, SDS_PRIVATE *psds, const int *ptr,
                int len) {
    int count, total = 0;
    ...
    while (total < len) {
        ...
        memcpy(&(psds->write_samples[psds->write_count]), &(ptr[total]),
        count * sizeof(int));
        total += count;
