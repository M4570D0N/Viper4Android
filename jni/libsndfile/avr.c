...
static int avr_read_header(SF_PRIVATE *psf) {
    ...
    psf_binheader_readf(psf, "pmb", 0, &hdr.marker, &hdr.name,
                    sizeof(hdr.name));
    ...
    psf_binheader_readf(psf, "E22222", &hdr.mono, &hdr.rez, &hdr.sign,
                    &hdr.loop, &hdr.midi);
    ...
    psf_binheader_readf(psf, "E4444", &hdr.srate, &hdr.frames, &hdr.lbeg,
                        &hdr.lend);
    ...
    psf_binheader_readf(psf, "bb", hdr.ext, sizeof(hdr.ext), hdr.user,
                    sizeof(hdr.user));
...
static int avr_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "Emz22", TWOBIT_MARKER, make_size_t(8),
                         psf->sf.channels == 2 ? 0xFFFF : 0,
                         psf->bytewidth * 8);

    sign = ((SF_CODEC (psf->sf.format)) == SF_FORMAT_PCM_U8) ? 0 : 0xFFFF;

    psf_binheader_writef(psf, "E222", sign, 0, 0xFFFF);
    psf_binheader_writef(psf, "E4444", psf->sf.samplerate, psf->sf.frames, 0,
                         0);

    psf_binheader_writef(psf, "E222zz", 0, 0, 0, make_size_t(20),
                         make_size_t(64));
