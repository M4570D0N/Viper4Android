...
static int w64_read_header(SF_PRIVATE *psf, int *blockalign,
                int *framesperblock);
...
int w64_open(SF_PRIVATE *psf) {
    ...
    if (psf->file.mode == SFM_WRITE || psf->file.mode == SFM_RDWR) {
        ...
        if (subformat == SF_FORMAT_IMA_ADPCM
            ...
            if (psf->sf.frames <= 0)
                psf->sf.frames = (psf->blockwidth) ?
                                psf->filelength / psf->blockwidth: psf->filelength;
...
static int w64_read_header(SF_PRIVATE *psf, int *blockalign, int *framesperblock) {
    ...
    while (!done) { /* Each new chunk must start on an 8 byte boundary, so jump if needed. */
    ...
    switch (format) {
        ...
        case WAVE_FORMAT_EXTENSIBLE:
            ...
            psf->sf.format = SF_FORMAT_W64
                            | u_bitwidth_to_subformat(psf->bytewidth * 8);
            break;
...
static int w64_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "eh8hh", riff_MARKER16, psf->filelength,
                    wave_MARKER16, fmt_MARKER16);
    ...
    switch (subformat) {
        ...
        case SF_FORMAT_PCM_32:
            ...
            psf_binheader_writef(psf, "e8224", fmt_size, WAVE_FORMAT_PCM,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "e4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
            /*  fmt : blockalign, bitwidth */
            psf_binheader_writef(psf, "e22", psf->bytewidth * psf->sf.channels,
                            psf->bytewidth * 8);
            break;
        ...
        case SF_FORMAT_DOUBLE:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "e8224", fmt_size, WAVE_FORMAT_IEEE_FLOAT,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "e4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
        ...
        case SF_FORMAT_ULAW:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "e8224", fmt_size, WAVE_FORMAT_MULAW,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "e4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
        ...
        case SF_FORMAT_ALAW:
            ...
            /* fmt : format, channels, samplerate */
            psf_binheader_writef(psf, "e8224", fmt_size, WAVE_FORMAT_ALAW,
                            psf->sf.channels, psf->sf.samplerate);
            /*  fmt : bytespersec */
            psf_binheader_writef(psf, "e4",
                            psf->sf.samplerate * psf->bytewidth * psf->sf.channels);
        ...
        case SF_FORMAT_IMA_ADPCM: {
            ...
            psf_binheader_writef(psf, "e822", fmt_size, WAVE_FORMAT_IMA_ADPCM,
                            psf->sf.channels);
        ...
        case SF_FORMAT_MS_ADPCM: {
            ...
            psf_binheader_writef(psf, "e822", fmt_size, WAVE_FORMAT_MS_ADPCM,
                            psf->sf.channels);
            ...
            /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
            psf_binheader_writef(psf, "e22222", blockalign, 4, extrabytes,
                            framesperblock, 7);
        ...
        case SF_FORMAT_GSM610: {
            ...
            bytespersec = (psf->sf.samplerate * WAV_W64_GSM610_BLOCKSIZE)
            ...
            /* fmt : size, WAV format type, channels. */
            psf_binheader_writef(psf, "e822", fmt_size, WAVE_FORMAT_GSM610,
                            psf->sf.channels);
            ...
            /* fmt : blockalign, bitwidth, extrabytes, framesperblock. */
            psf_binheader_writef(psf, "e2222", WAV_W64_GSM610_BLOCKSIZE, 0, 2,
                            WAV_W64_GSM610_SAMPLES);
        };
    ...
    if (add_fact_chunk)
        psf_binheader_writef(psf, "eh88", fact_MARKER16,
                        (sf_count_t) (16 + 8 + 8), psf->sf.frames);
