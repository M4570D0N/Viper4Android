...
WavReader_R32::~WavReader_R32() {
    if (m_fpSamples != NULL) delete [] m_fpSamples;
    if (m_pFileHandle != NULL) fclose(m_pFileHandle);
    m_fpSamples = NULL;
    m_pFileHandle = NULL;
    m_uiFrameCount = 0;
    m_uiSamplingRate = 0;
...
bool WavReader_R32::ReadFOURCC(int8_t cCode[4]) {
    if (m_pFileHandle == NULL) return false;
    uint32_t nReadSize = (uint32_t)fread(cCode, sizeof(int8_t), 4, m_pFileHandle);
    if (nReadSize != 4) return false;
    return true;
}
...
void WavReader_R32::ConvertInt16ToFloat32(int16_t *ptrInput, int32_t nSamplesCount, int32_t nChannels, float *ptrOutput) {
    for (int32_t i = 0; i < nSamplesCount * nChannels; i++)
        ptrOutput[i] = (float)((double)((int16_t)(ptrInput[i])) * 0.000030517578125);
}

void WavReader_R32::ConvertInt24ToFloat32(uint8_t *ptrInput, int32_t nSamplesCount, int32_t nChannels, float *ptrOutput) {
    for (int32_t i = 0; i < nSamplesCount * nChannels; i++) {
...
void WavReader_R32::ConvertInt32ToFloat32(int32_t *ptrInput, int32_t nSamplesCount, int32_t nChannels, float *ptrOutput) {
    for (int32_t i = 0; i < nSamplesCount * nChannels; i++)
...
bool WavReader_R32::OpenWavFile(const char *pszFilePathName) {
    if (pszFilePathName == NULL) return false;
    if (strlen(pszFilePathName) <= 0) return false;
    if (m_fpSamples != NULL) delete [] m_fpSamples;
    if (m_pFileHandle != NULL) fclose(m_pFileHandle);
    m_fpSamples = NULL;
    m_pFileHandle = NULL;
    ...
    if (nFormatLength < 16) return false;
    uint16_t nFormatTag = ReadUINT16();
    if ((nFormatTag != 0x0001) && (nFormatTag != 0x0003)) return false;
    ...
    if (nFormatTag == 0x0003) {
        ...
        if (m_fpSamples == NULL) return false;
        uint32_t nReadFrameCount = (uint32_t)fread(m_fpSamples, sizeof(float), uiFrameCount * uiChannels, m_pFileHandle);
        ...
        uint32_t nReadFrameCount = (uint32_t)fread(ucFrameBuffer, sizeof(uint8_t), uiReadDataLength, m_pFileHandle);
        ...
        switch (uiBitsPerSample) {
            case 8:
            ConvertInt8ToFloat32(ucFrameBuffer, uiFrameCount, uiChannels, m_fpSamples);
            break;

            case 16:
            ConvertInt16ToFloat32((int16_t*)ucFrameBuffer, uiFrameCount, uiChannels, m_fpSamples);
            break;

            case 24:
            ConvertInt24ToFloat32(ucFrameBuffer, uiFrameCount, uiChannels, m_fpSamples);
            break;

            case 32:
            ConvertInt32ToFloat32((int32_t*)ucFrameBuffer, uiFrameCount, uiChannels, m_fpSamples);
            break;
        }
        delete [] ucFrameBuffer;
        m_uiFrameCount = uiFrameCount;
...
uint32_t WavReader_R32::ReadEntirePCM(float *fpBuffer, uint32_t nFrames) {
    if (m_fpSamples == NULL) return 0;
    if (fpBuffer == NULL) return 0;
    if (nFrames != m_uiFrameCount) return 0;
    memcpy(fpBuffer, m_fpSamples, m_uiFrameCount * m_uiChannels * sizeof(float));
    return nFrames;
}
