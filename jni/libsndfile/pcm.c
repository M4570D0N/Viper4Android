...
static sf_count_t pcm_write_s2sc(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2uc(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2bes(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2les(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2bet(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2let(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2bei(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t pcm_write_s2lei(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);

static sf_count_t pcm_write_i2sc(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2uc(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2bes(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2les(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2bet(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2let(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2bei(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t pcm_write_i2lei(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);

static sf_count_t pcm_write_f2sc(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2uc(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2bes(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2les(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2bet(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2let(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2bei(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t pcm_write_f2lei(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);

static sf_count_t pcm_write_d2sc(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2uc(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2bes(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2les(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2bet(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2let(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2bei(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
static sf_count_t pcm_write_d2lei(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);

/*-----------------------------------------------------------------------------------------------
 */

enum { /* Char type for 8 bit files. */
    SF_CHARS_SIGNED = 200, SF_CHARS_UNSIGNED = 201
};
...
static inline void sc2s_array(signed char *src, int count, short *dest) {
    while (--count >= 0) {
        dest[count] = src[count] << 8;
    };
} /* sc2s_array */

static inline void uc2s_array(unsigned char *src, int count, short *dest) {
    while (--count >= 0) {
        dest[count] = (((short) src[count]) - 0x80) << 8;
    };
} /* uc2s_array */

static inline void let2s_array(tribyte *src, int count, short *dest) {
    unsigned char *ucptr;
...
static inline void bet2s_array(tribyte *src, int count, short *dest) {
    unsigned char *ucptr;
...
static inline void lei2s_array(int *src, int count, short *dest) {
    int value;
...
static inline void bei2s_array(int *src, int count, short *dest) {
    int value;
...
static inline void sc2i_array(signed char *src, int count, int *dest) {
    while (--count >= 0) {
        dest[count] = ((int) src[count]) << 24;
    };
} /* sc2i_array */

static inline void uc2i_array(unsigned char *src, int count, int *dest) {
    while (--count >= 0) {
        dest[count] = (((int) src[count]) - 128) << 24;
    };
} /* uc2i_array */

static inline void bes2i_array(short *src, int count, int *dest) {
    short value;
...
static inline void les2i_array(short *src, int count, int *dest) {
    short value;
...
static inline void bet2i_array(tribyte *src, int count, int *dest) {
    unsigned char *ucptr;
...
static inline void let2i_array(tribyte *src, int count, int *dest) {
    unsigned char *ucptr;
...
static inline void sc2f_array(signed char *src, int count, float *dest,
                float normfact) {
    while (--count >= 0)
        dest[count] = ((float) src[count]) * normfact;
} /* sc2f_array */

static inline void uc2f_array(unsigned char *src, int count, float *dest,
                float normfact) {
    while (--count >= 0)
        dest[count] = (((int) src[count]) - 128) * normfact;
} /* uc2f_array */

static inline void les2f_array(short *src, int count, float *dest,
                float normfact) {
    short value;
...
static inline void bes2f_array(short *src, int count, float *dest,
                float normfact) {
    short value;
...
static inline void let2f_array(tribyte *src, int count, float *dest,
                float normfact) {
    unsigned char *ucptr;
    int value;
...
static inline void bet2f_array(tribyte *src, int count, float *dest,
                float normfact) {
    unsigned char *ucptr;
    int value;
...
static inline void lei2f_array(int *src, int count, float *dest, float normfact) {
    int value;
...
static inline void bei2f_array(int *src, int count, float *dest, float normfact) {
    int value;
...
static inline void sc2d_array(signed char *src, int count, double *dest,
                double normfact) {
    while (--count >= 0)
        dest[count] = ((double) src[count]) * normfact;
} /* sc2d_array */

static inline void uc2d_array(unsigned char *src, int count, double *dest,
                double normfact) {
    while (--count >= 0)
        dest[count] = (((int) src[count]) - 128) * normfact;
} /* uc2d_array */

static inline void les2d_array(short *src, int count, double *dest,
                double normfact) {
    short value;
...
static inline void bes2d_array(short *src, int count, double *dest,
                double normfact) {
    short value;
...
static inline void let2d_array(tribyte *src, int count, double *dest,
                double normfact) {
    unsigned char *ucptr;
    int value;
...
static inline void bet2d_array(tribyte *src, int count, double *dest,
                double normfact) {
    unsigned char *ucptr;
    int value;
...
static inline void lei2d_array(int *src, int count, double *dest,
                double normfact) {
    int value;
...
static inline void bei2d_array(int *src, int count, double *dest,
                double normfact) {
    int value;
...
static inline void s2sc_array(const short *src, signed char *dest, int count) {
    while (--count >= 0)
        dest[count] = src[count] >> 8;
} /* s2sc_array */

static inline void s2uc_array(const short *src, unsigned char *dest, int count) {
    while (--count >= 0)
        dest[count] = (src[count] >> 8) + 0x80;
} /* s2uc_array */

static inline void s2let_array(const short *src, tribyte *dest, int count) {
    unsigned char *ucptr;
...
static inline void s2bet_array(const short *src, tribyte *dest, int count) {
    unsigned char *ucptr;
...
static inline void s2lei_array(const short *src, int *dest, int count) {
    unsigned char *ucptr;
...
static inline void s2bei_array(const short *src, int *dest, int count) {
    unsigned char *ucptr;
...
static inline void i2sc_array(const int *src, signed char *dest, int count) {
    while (--count >= 0)
        dest[count] = (src[count] >> 24);
} /* i2sc_array */

static inline void i2uc_array(const int *src, unsigned char *dest, int count) {
    while (--count >= 0)
        dest[count] = ((src[count] >> 24) + 128);
} /* i2uc_array */

static inline void i2bes_array(const int *src, short *dest, int count) {
    unsigned char *ucptr;
...
static inline void i2les_array(const int *src, short *dest, int count) {
    unsigned char *ucptr;
...
static inline void i2let_array(const int *src, tribyte *dest, int count) {
    unsigned char *ucptr;
    int value;
...
static inline void i2bet_array(const int *src, tribyte *dest, int count) {
    unsigned char *ucptr;
    int value;
...
static sf_count_t pcm_read_sc2s(SF_PRIVATE *psf, short *ptr, sf_count_t len) {
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.scbuf, sizeof(signed char), bufferlen,
                        psf);
...
static sf_count_t pcm_read_uc2s(SF_PRIVATE *psf, short *ptr, sf_count_t len) {
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.ucbuf, sizeof(unsigned char), bufferlen,
                        psf);
...
static sf_count_t pcm_read_bet2f(SF_PRIVATE *psf, float *ptr, sf_count_t len) {
    ...
    normfact = (psf->norm_float == SF_TRUE) ?
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
        bet2f_array((tribyte*) (psf->u.ucbuf), readcount, ptr + total,
                        normfact);
        total += readcount;
        if (readcount < bufferlen)
            break;
...
static sf_count_t pcm_read_let2f(SF_PRIVATE *psf, float *ptr, sf_count_t len) {
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
        let2f_array((tribyte*) (psf->u.ucbuf), readcount, ptr + total,
                        normfact);
        total += readcount;
        if (readcount < bufferlen)
            break;
...
static sf_count_t pcm_read_bet2d(SF_PRIVATE *psf, double *ptr, sf_count_t len) {
    ...
    normfact = (psf->norm_double == SF_TRUE) ?
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
        bet2d_array((tribyte*) (psf->u.ucbuf), readcount, ptr + total,
                        normfact);
        total += readcount;
        if (readcount < bufferlen)
            break;
...
static sf_count_t pcm_read_let2d(SF_PRIVATE *psf, double *ptr, sf_count_t len) {
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        readcount = psf_fread(psf->u.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
        let2d_array((tribyte*) (psf->u.ucbuf), readcount, ptr + total,
                        normfact);
        total += readcount;
        if (readcount < bufferlen)
            break;
...
static sf_count_t pcm_write_s2sc(SF_PRIVATE *psf, const short *ptr,
    ...
    while (len > 0) {
        ...
        writecount = psf_fwrite(psf->u.scbuf, sizeof(signed char), bufferlen,
                        psf);
        total += writecount;
...
static sf_count_t pcm_write_s2uc(SF_PRIVATE *psf, const short *ptr,
    ...
    while (len > 0) {
        ...
        writecount = psf_fwrite(psf->u.ucbuf, sizeof(unsigned char), bufferlen,
                        psf);
        total += writecount;
        if (writecount < bufferlen)
            break;
...
static sf_count_t pcm_write_i2uc(SF_PRIVATE *psf, const int *ptr,
    ...
    while (len > 0) {
        ...
        writecount = psf_fwrite(psf->u.ucbuf, sizeof(signed char), bufferlen,
                        psf);
        total += writecount;
        if (writecount < bufferlen)
            break;
...
static void f2sc_array(const float *src, signed char *dest, int count,
                int normalize) {
    float normfact;
...
static void f2sc_clip_array(const float *src, signed char *dest, int count,
                int normalize) {
...
static void f2uc_array(const float *src, unsigned char *dest, int count,
                int normalize) {
    float normfact;
...
static void f2uc_clip_array(const float *src, unsigned char *dest, int count,
                int normalize) {
    float normfact, scaled_value;
...
static void f2bes_array(const float *src, short *dest, int count, int normalize) {
    unsigned char *ucptr;
    float normfact;
    short value;
...
static void f2bes_clip_array(const float *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static void f2les_array(const float *src, short *dest, int count, int normalize) {
    unsigned char *ucptr;
    float normfact;
    int value;
...
static void f2les_clip_array(const float *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static void f2let_array(const float *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact;
    int value;
...
static void f2let_clip_array(const float *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static sf_count_t pcm_write_f2let(SF_PRIVATE *psf, const float *ptr,
    ...
    while (len > 0) {
        if (len < bufferlen)
            bufferlen = (int) len;
        convert(ptr + total, (tribyte*) (psf->u.ucbuf), bufferlen,
                        psf->norm_float);
        writecount = psf_fwrite(psf->u.ucbuf, SIZEOF_TRIBYTE, bufferlen, psf);
        total += writecount;
        if (writecount < bufferlen)
...
static void f2bet_array(const float *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact;
    int value;
...
static void f2bet_clip_array(const float *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static void f2bei_array(const float *src, int *dest, int count, int normalize) {
    unsigned char *ucptr;
    float normfact;
    int value;
...
static void f2bei_clip_array(const float *src, int *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static void f2lei_array(const float *src, int *dest, int count, int normalize) {
    unsigned char *ucptr;
    float normfact;
    int value;
...
static void f2lei_clip_array(const float *src, int *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    float normfact, scaled_value;
    int value;
...
static void d2sc_array(const double *src, signed char *dest, int count,
                int normalize) {
    double normfact;
...
static void d2sc_clip_array(const double *src, signed char *dest, int count,
                int normalize) {
...
static void d2uc_array(const double *src, unsigned char *dest, int count,
                int normalize) {
    double normfact;
...
static void d2uc_clip_array(const double *src, unsigned char *dest, int count,
                int normalize) {
    double normfact, scaled_value;
...
static void d2bes_array(const double *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    short value;
    double normfact;
...
static void d2bes_clip_array(const double *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    double normfact, scaled_value;
    int value;
...
static void d2les_array(const double *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    short value;
    double normfact;
...
static void d2les_clip_array(const double *src, short *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact, scaled_value;
...
static void d2let_array(const double *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact;
...
static void d2let_clip_array(const double *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact, scaled_value;
...
static void d2bet_array(const double *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact;
...
static void d2bet_clip_array(const double *src, tribyte *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact, scaled_value;
...
static void d2bei_array(const double *src, int *dest, int count, int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact;
...
static void d2bei_clip_array(const double *src, int *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact, scaled_value;
...
static void d2lei_array(const double *src, int *dest, int count, int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact;
...
static void d2lei_clip_array(const double *src, int *dest, int count,
                int normalize) {
    unsigned char *ucptr;
    int value;
    double normfact, scaled_value;
