...
static int caf_command(SF_PRIVATE * psf, int command, void * UNUSED (data),
    ...
    switch (command) {
        case SFC_SET_CHANNEL_MAP_INFO:
            pcaf->chanmap_tag = aiff_caf_find_channel_layout_tag(
                    psf->channel_map, psf->sf.channels);
...
static int decode_desc_chunk(SF_PRIVATE *psf, const DESC_CHUNK *desc) {
    ...
    format = SF_FORMAT_CAF
            | (psf->endian == SF_ENDIAN_LITTLE ? SF_ENDIAN_LITTLE : 0);
...
static int caf_read_header(SF_PRIVATE *psf) {
    ...
    psf_binheader_readf(psf, "mE44444", &desc.fmt_id, &desc.fmt_flags,
                    &desc.pkt_bytes, &desc.pkt_frames,
                    &desc.channels_per_frame, &desc.bits_per_chan);
    psf_log_printf(psf,
            "  Format id    : %M\n  Format flags : %x\n  Bytes / packet   : %u\n"
            "  Frames / packet  : %u\n  Channels / frame : %u\n  Bits / channel   : %u\n",
            desc.fmt_id, desc.fmt_flags, desc.pkt_bytes, desc.pkt_frames,
            desc.channels_per_frame, desc.bits_per_chan);
    ...
    while (have_data == 0
            && psf_ftell(psf) < psf->filelength - SIGNED_SIZEOF (marker) ) {
        ...
        switch (marker) {
            case peak_MARKER:
                ...
                if (chunk_size != CAF_PEAK_CHUNK_SIZE (psf->sf.channels) ) {
                    ...
                    psf_log_printf(psf,
                                   "*** File PEAK chunk %D should be %d.\n",
                                   chunk_size,
                                   CAF_PEAK_CHUNK_SIZE (psf->sf.channels) );
                    return SFE_CAF_BAD_PEAK;
                };

                if ((psf->peak_info = peak_info_calloc(psf->sf.channels))
                        == NULL)
                    return SFE_MALLOC_FAILED;
            ...
            case data_MARKER:
                if (psf->filelength > 0
                        && chunk_size + psf->headindex != psf->filelength)
...
static int caf_write_header(SF_PRIVATE *psf, int calc_length) {
    ...
    psf_binheader_writef(psf, "Em8", desc_MARKER,
    ...
    if (CPU_IS_BIG_ENDIAN && (psf->endian == 0 || psf->endian == SF_ENDIAN_CPU))
        psf->endian = SF_ENDIAN_BIG;
            ...
            && (psf->endian == SF_ENDIAN_LITTLE || psf->endian == SF_ENDIAN_CPU))
    ...
    psf_binheader_writef(psf, "mE44444", desc.fmt_id, desc.fmt_flags,
                     desc.pkt_bytes, desc.pkt_frames,
                     desc.channels_per_frame, desc.bits_per_chan);
    ...
    if (psf->peak_info != NULL) {
        ...
        psf_binheader_writef(
                psf, "Em84", peak_MARKER,
                (sf_count_t) CAF_PEAK_CHUNK_SIZE (psf->sf.channels),
                psf->peak_info->edit_number);
        for (k = 0; k < psf->sf.channels; k++)
    ...
    if (psf->channel_map && pcaf->chanmap_tag)
        psf_binheader_writef(psf, "Em8444", chan_MARKER, (sf_count_t) 12,
                             pcaf->chanmap_tag, 0, 0);
