...
static sf_count_t ulaw_write_s2ulaw(SF_PRIVATE *psf, const short *ptr,
                sf_count_t len);
static sf_count_t ulaw_write_i2ulaw(SF_PRIVATE *psf, const int *ptr,
                sf_count_t len);
static sf_count_t ulaw_write_f2ulaw(SF_PRIVATE *psf, const float *ptr,
                sf_count_t len);
static sf_count_t ulaw_write_d2ulaw(SF_PRIVATE *psf, const double *ptr,
                sf_count_t len);
...
static inline void ulaw2s_array(unsigned char *buffer, int count, short *ptr) {
    while (--count >= 0)
        ptr[count] = ulaw_decode[(int) buffer[count]];
} /* ulaw2s_array */

static inline void ulaw2i_array(unsigned char *buffer, int count, int *ptr) {
    while (--count >= 0)
        ptr[count] = ulaw_decode[buffer[count]] << 16;
} /* ulaw2i_array */

static inline void ulaw2f_array(unsigned char *buffer, int count, float *ptr,
                float normfact) {
    while (--count >= 0)
        ptr[count] = normfact * ulaw_decode[(int) buffer[count]];
} /* ulaw2f_array */

static inline void ulaw2d_array(const unsigned char *buffer, int count,
                double *ptr, double normfact) {
    while (--count >= 0)
        ptr[count] = normfact * ulaw_decode[(int) buffer[count]];
} /* ulaw2d_array */

static inline void s2ulaw_array(const short *ptr, int count,
...
static inline void f2ulaw_array(const float *ptr, int count,
