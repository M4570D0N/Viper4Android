...
typedef struct {
    /* internal : write a PEAK chunk at the start or end of the file? */
    int peak_loc;
     ...
     ** yourself a 1999 ISO C standards compliant compiler. GCC-3.X is
     ** highly recommended.
     */
    PEAK_POS peaks [0];
#endif
...
typedef SF_BROADCAST_INFO_VAR (16 * 1024)
...
typedef struct sf_private_tag {
    ...
    sf_count_t (*read_short)(struct sf_private_tag*, short *ptr,
                    sf_count_t len);
    sf_count_t (*read_int)(struct sf_private_tag*, int *ptr,
                    sf_count_t len);
    sf_count_t (*read_float)(struct sf_private_tag*, float *ptr,
                    sf_count_t len);
    sf_count_t (*read_double)(struct sf_private_tag*, double *ptr,
                    sf_count_t len);
    sf_count_t (*write_short)(struct sf_private_tag*, const short *ptr,
                    sf_count_t len);
    sf_count_t (*write_int)(struct sf_private_tag*, const int *ptr,
                    sf_count_t len);
    sf_count_t (*write_float)(struct sf_private_tag*, const float *ptr,
                    sf_count_t len);
    sf_count_t (*write_double)(struct sf_private_tag*, const double *ptr,
                    sf_count_t len);

    sf_count_t (*seek)(struct sf_private_tag*, int mode,
                    sf_count_t samples_from_start);
    int (*write_header)(struct sf_private_tag*, int calc_length);
    int (*command)(struct sf_private_tag*, int command, void *data,
                    int datasize);

    /* Separate close functions for the codec and the container.
     * he codec close function is always called first.
     */
...
void psf_strlcpy_crlf(char *dest, const char *src, size_t destmax,
                size_t srcmax);
...
sf_count_t psf_default_seek(SF_PRIVATE *psf, int mode,
                sf_count_t samples_from_start);
...
sf_count_t psf_fread(void *ptr, sf_count_t bytes, sf_count_t count,
                SF_PRIVATE *psf);
sf_count_t psf_fwrite(const void *ptr, sf_count_t bytes, sf_count_t count,
                SF_PRIVATE *psf);
...
void pchk4_store(PRIV_CHUNK4 * pchk, int marker, sf_count_t offset,
                sf_count_t len);
...
int broadcast_var_set(SF_PRIVATE *psf, const SF_BROADCAST_INFO * data,
                size_t datasize);
int broadcast_var_get(SF_PRIVATE *psf, SF_BROADCAST_INFO * data,
                size_t datasize);
