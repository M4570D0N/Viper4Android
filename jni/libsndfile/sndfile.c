...
SNDFILE* sf_open_virtual(SF_VIRTUAL_IO *sfvirtual, int mode, SF_INFO *sfinfo,
    ...
    if (sfvirtual->get_filelen == NULL || sfvirtual->seek == NULL
                    || sfvirtual->tell == NULL) {
        sf_errno = SFE_BAD_VIRTUAL_IO;
        snprintf(sf_logbuffer,
                 sizeof(sf_logbuffer),
    ...
    if ((mode == SFM_READ || mode == SFM_RDWR) && sfvirtual->read == NULL) {
        sf_errno = SFE_BAD_VIRTUAL_IO;
        snprintf(sf_logbuffer, sizeof(sf_logbuffer),
...
int sf_command(SNDFILE *sndfile, int command, void *data, int datasize) {
    ...
    switch (command) {
        ...
        case SFC_GET_CHANNEL_MAP_INFO:
            ...
            if (data == NULL
                            || datasize != SIGNED_SIZEOF (psf->channel_map [0])
                            * psf->sf.channels) {
                psf->error = SFE_BAD_COMMAND_PARAM;
                return SF_FALSE;
            };
        ...
        case SFC_SET_CHANNEL_MAP_INFO:
...

            {
                ...
                for (iptr = data; iptr < (int*) data + psf->sf.channels;
                                iptr++) {
                    if (*iptr <= SF_CHANNEL_MAP_INVALID
                                    || *iptr >= SF_CHANNEL_MAP_MAX) {
                        psf->error = SFE_BAD_COMMAND_PARAM;
                        return SF_FALSE;
                    };
                };
...
sf_count_t sf_writef_float(SNDFILE *sndfile, const float *ptr,
                           sf_count_t frames) {
    SF_PRIVATE *psf;
    sf_count_t count;
...
sf_count_t sf_writef_double(SNDFILE *sndfile, const double *ptr,
                            sf_count_t frames) {
    SF_PRIVATE *psf;
    sf_count_t count;
...
static int guess_file_type(SF_PRIVATE *psf) {
    ...
    if (psf_binheader_readf(psf, "b", &buffer,
                            SIGNED_SIZEOF (buffer) ) != SIGNED_SIZEOF (buffer)) {
    ...
    if ((buffer[0] == MAKE_MARKER ('R', 'I', 'F', 'F')
                    ...
                    && buffer [2] == MAKE_MARKER ('W', 'A', 'V', 'E'))return SF_FORMAT_WAV;
    ...
    if ((buffer[0] & MAKE_MARKER (0xFF, 0xFF, 0xF8, 0xFF))== MAKE_MARKER (0x64, 0xA3, 0x00, 0x00) ||
                    (buffer [0] & MAKE_MARKER (0xFF, 0xF8, 0xFF, 0xFF)) == MAKE_MARKER (0x00, 0x00, 0xA3, 0x64))return SF_FORMAT_IRCAM;

    if (buffer[0] == MAKE_MARKER ('r', 'i', 'f', 'f'))
        return SF_FORMAT_W64;

    if (buffer[0] == MAKE_MARKER (0, 0, 0x03, 0xE8)
                    && buffer[1] == MAKE_MARKER (0, 0, 0, 1)
                    && buffer[2] == MAKE_MARKER (0, 0, 0, 1))
        return SF_FORMAT_MAT4;

    if (buffer[0] == MAKE_MARKER (0, 0, 0, 0)
                    && buffer[1] == MAKE_MARKER (1, 0, 0, 0)
    ...
    if (2 * BEI2H_INT (buffer [0]) + 12
                    == psf->filelength&& buffer [2] == MAKE_MARKER (0, 2, 0, 0))return SF_FORMAT_HTK;
...
static void copy_filename(SF_PRIVATE *psf, const char *path) {
    ...
    if ((cptr = strrchr(psf->file.dir.c, '/'))
                    || (cptr = strrchr(psf->file.dir.c, '\\')))
        cptr[1] = 0;
    else
        psf->file.dir.c[0] = 0;
...
SNDFILE * psf_open_file(SF_PRIVATE *psf, SF_INFO *sfinfo) {
    ...
    if (psf->file.mode != SFM_READ && psf->file.mode != SFM_WRITE
    ...
    error_exit: sf_errno = error;

    if (error == SFE_SYSTEM) {
        snprintf(sf_syserr, sizeof(sf_syserr), "%s", psf->syserr);
        LOGI("SoundFile: SystemError = %s", sf_syserr);
    }
    snprintf(sf_logbuffer, sizeof(sf_logbuffer), "%s", psf->logbuffer);
    LOGI("SoundFile: LogBuffer = %s", sf_logbuffer);
    ...
    LOGI("SoundFile: OpenFile() failed, errcode = %d", error);
    psf_close(psf);
    return NULL;
} /* psf_open_file */
