...
sf_count_t psf_fread(void *ptr, sf_count_t bytes, sf_count_t items,
    ...
    while (items > 0) { /* Break the read down to a sensible size. */
...
sf_count_t psf_fwrite (const void *ptr, sf_count_t bytes,
    ...
    while (items > 0) {
      /* Break the writes down to a sensible size. */
        ...
        if (WriteFile (psf->file.handle, ((const char*) ptr)
                        + total, count, &dwNumberOfBytesWritten, 0) == 0) {
            psf_log_syserr (psf, GetLastError ());
            break;
        } else
...
/* Win32 */
int psf_fopen (SF_PRIVATE *psf, const char *pathname, int open_mode) {
    int oflag, mode;
